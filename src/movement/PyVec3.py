...
class PyVec3:
    ...
    def assign(self, *args):
        if len(args) == 1:
            ...
            if type(arg) in PyVec3.ScalarTypes:
                ...
                x = arg.getX(); y = arg.getY(); z = arg.getZ()
            else:
                raise TypeError
        elif len(args) == 3:
    ...
    def fill(self, s): self.x = self.y = self.z = s
    ...
    # this implementation differs from Panda VBase3 (componentwise, L to R)
    def __lt__(a, b):
    ...
    def __iadd__(self, other):
        self.x += other.getX(); self.y += other.getY(); self.z += other.getZ()
        return self
